...
def documentation():
    ...
    """
    return jsonify({
        'title': 'Emotion Detection API Documentation',
        'description': 'API for detecting emotions from text using IBM Watson NLP',
        'base_url': 'http://localhost:5000',
        'endpoints': [
            {
                'path': '/emotion',
                'method': 'POST',
                'description': 'Detects emotions from provided text',
                'request_body': {
                    'type': 'application/json',
                    'example': {'text': 'I am very happy!'}
                },
                'response': {
                    'status': 200,
                    'example': {
